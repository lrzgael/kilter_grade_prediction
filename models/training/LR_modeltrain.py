...
# -*- coding: utf-8 -*-
"""
Training script for the Kilter Board difficulty model (Linear Regression).

Input : data/processed/data_processed_train.csv
        data/processed/data_processed_test.csv   (optional, only for quick sanity check)
Output: models/saved_models/LR_model.joblib

Usage:
    python models/training/LR_modeltrain.py --train-csv data/processed/data_processed_train.csv \
        --test-csv data/processed/data_processed_test.csv --model-out models/saved_models/LR_model.joblib
...
def train_model(X_train, y_train, w_train, random_state=DEFAULT_RANDOM_STATE):
    """Train an ordinary least-squares linear regression (hard baseline)."""
...
def tune_model(X_train, y_train, w_train, *, groups=None, n_iter=20,
    ...
    """Randomized search over Ridge regularization strength and solver."""
    ...
    final_model = Ridge(**search.best_params_, random_state=random_state)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train the Linear Regression Kilter grade prediction model.",
    ...
    parser.add_argument("--model-out", type=Path,
                        default="models/saved_models/LR_model.joblib",
...
def main():
    ...
    # #Train the model (only for tests)
    # print("\nTraining LinearRegression (OLS baseline)...")
